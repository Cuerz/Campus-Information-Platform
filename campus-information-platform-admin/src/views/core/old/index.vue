<template>
       ...
       <el-col :span="20" :xs="24" :offset="2">
         ...
         >
           <el-form-item label="物品名称" prop="oldStuffName">
             <el-input
               v-model="queryParams.oldStuffName"
         ...
         >
           ...
           >
             <template #default="scope">
               <el-image
                 :src="scope.row.oldStuffPic"
                 :preview-src-list="[scope.row.oldStuffPic]"
                 preview-teleported
               ></el-image>
             </template>
           </el-table-column>
         ...
         <el-row>
           <el-col :span="12">
             <el-form-item label="物品图片" prop="oldStuffPic">
               ...
               >
                 <img v-if="form.oldStuffPic" :src="form.oldStuffPic" class="img" />
 ...
 function handleSelectionChange(selection) {
   ...
   idSelect.value = selection.map((item) => item.oldStuffId);
 }
 
 /** 查询失物列表 */
 function getList() {
   loading.value = true;
   listOldStuff(queryParams.value).then((res) => {
     oldStuffList.value = res.data.list;
 ...
 function handleExamineChange(row) {
   ...
   proxy.$modal
     .confirm('确认审核"' + text + '"' + '编号为"' + row.oldStuffId + '"的记录吗?')
     .then(function () {
       return changeOldExamine(row.oldStuffId, row.examine);
     })
     .then(() => {
 ...
 function handleDelete(row) {
   const oldStuffId = row.oldStuffId || idSelect.value[0];
   proxy.$modal
     .confirm('是否确认删除编号为"' + oldStuffId + '"的记录？')
     .then(function () {
       return delStuff(oldStuffId);
     })
     .then(() => {
       getList();
